...
(function () {
    ...
    angular.module('BlurAdmin.pages.monitoring')
        .controller('monitoringolrightctrl', monitoringolrightctrl);
  
    /** @ngInject */
    function monitoringolrightctrl($state, $scope, $filter, monitoringService, editableOptions, editableThemes, managequestionaireService, manageparticipantService) {
        var vm = this;
        vm.searchParam = {
        ...
        vm.actionSelect = function (params) {
            $state.go('monitoring.olrightdetail', {questionaire: params});
        };
        vm.changeTypeQuestionnaire = function  (){
            vm.searchParam.questType = vm.filterquestType.value; 
            if(vm.filterquestType.label != 'olright')
            {vm.filterquartal = null; vm.searchParam.quartal = null};
        }
        vm.gridQuestionaire = {
        ...
        vm.refreshGrid = function () {
            monitoringService.getmonitoringolright(vm.searchParam).then(function (result) {
