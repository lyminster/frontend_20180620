...
(function () {
    ...
    /** @ngInject */
    function createparticipantctrl($q, $state, $scope, $uibModal, toastr, $filter, editableOptions, editableThemes, fillingquestionaireService, managequestionaireService, authService, $stateParams, localStorageService) {
        var vm = this;
        ...
        vm.title = 'New ' + vm.type + ' Participant (' + vm.questionaire.questionnaireName + ')';
        var promises = [];
        ...
        vm.saveParticipant = function () {
            if (!vm.formmodel.$invalid) {
                if (vm.isdcsl) {
                    fillingquestionaireService.saveScreeningNewParticipant(vm.newParticipant).then(function (result) {
                        if (result.statusText == "OK") {
                            toastr.success('Data berhasil di simpan', 'Success');
                            $state.go('fillingquestionaire.filling', { questionaire: vm.questionaire, idparticipant: result.data });
                        } else {
                    ...
                    fillingquestionaireService.saveScreeningNewParticipantOlright(vm.newParticipant).then(function (result) {
                        if (result.statusText == "OK") {
                            toastr.success('Data berhasil di simpan', 'Success');
                            $state.go('fillingquestionaire.fillingolright', { questionaire: vm.questionaire, idparticipant: result.data.participantID, data: result.data });
                        } else {
                            toastr.error(result.message, 'Error');
                        }
                    });
