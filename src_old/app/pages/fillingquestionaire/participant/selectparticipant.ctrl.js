...
(function () {
    ...
    /** @ngInject */
    function selectparticipantctrl($state, $scope, monitoringService, $uibModal, toastr, $filter, editableOptions, editableThemes, fillingquestionaireService, authService, $stateParams) {
        var vm = this;
        ...
        vm.gridAvailableParticipant = {
            ...
            columnDefs: [
                ...
                {
                    name: 'action', visible: vm.isdcsl, enableFiltering: false,  cellTemplate: '<div ng-disabled="row.entity.status != \'2\'" style="display: flex;align-items: center;justify-content: center;"><button type="button" ng-click="grid.appScope.vm.actionSelect(row.entity)" class="btn btn-success  btn-xs"><i class="ion-android-checkbox-outline"></i></button></div>', width: 100
                },
                { name: 'id', displayName: "Id", width: 50, visible: false },
                { name: 'title', displayName: "title", width: 50 },
                { name: 'name', displayName: "Name", width: 200 },
                { name: 'telephone', displayName: "Telephone",visible: vm.isdcsl, width: 200 },
                { name: 'email', displayName: "Email",visible: vm.isdcsl, width: 200 },
                { name: 'city', displayName: "City",visible: vm.isdcsl, width: 200 },
                { name: 'source', displayName: "Source",visible: vm.isdcsl, width: 100 },
                { name: 'typeSurvey', displayName: "Type Survey", width: 100 },
                { name: 'h1', displayName: "H1", width: 50 , visible: vm.isdcsl, cellTemplate: '<div style="display: flex;align-items: center;justify-content: center;"><input type="checkbox" ng-model="row.entity.h1" disabled></div>' },
                { name: 'h2', displayName: "H2", width: 50 , visible: vm.isdcsl, cellTemplate: '<div style="display: flex;align-items: center;justify-content: center;"><input type="checkbox" ng-model="row.entity.h2" disabled></div>' },
                { name: 'h3', displayName: "H3", width: 50 , visible: vm.isdcsl, cellTemplate: '<div style="display: flex;align-items: center;justify-content: center;"><input type="checkbox" ng-model="row.entity.h3" disabled></div>'},
        ...
        vm.actionSelect = function (row) {
            $state.go('fillingquestionaire.filling', {questionaire: vm.questionaire, idparticipant: row.id});
        }
        vm.createNewParticipant = function () {
            $state.go('fillingquestionaire.createparticipant', {questionaire: vm.questionaire, type: vm.type});
        }
    }
})();
